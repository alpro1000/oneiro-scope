...
- [x] Ревью ботов PR#137: cwd-обход ограничения закрыт (корень проекта
      ...
      (2026-07-05)
- [x] Ревью ботов PR#138: относительные output_path/photo_path
      детерминированно якорятся к корню проекта (не зависят от cwd);
      тест пустого output_path усилен до проверки tempdir; imread
      читает заякоренный путь (2026-07-05)
- [x] Двухслойные отчёты (запрос owner 2026-07-05): полный нарратив →
      тезисы для запоминания → данные с источниками; композиция
      детерминированная (0.6), без генерации
