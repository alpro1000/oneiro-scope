...
- [ ] Bэклог (Qodo, информационно): поиск корня проекта по
      ...
      структуры репо
- [x] Живой кейс 2026-07-05 (владелец поймал противоречие): метрика
      lip_fullness (внутренний зазор губ 13/14) меряет раскрытие рта,
      а не толщину — «экспрессивный рот» усреднился в «тонкие губы».
      Чтения mouth_thin/full от геометрии отключены (анкета — можно)
- [x] Ревью ботов PR#145 (Qodo, real): в смешанном режиме
      (метрики+анкета) skip_measurable отбрасывал lip_fullness из
      анкеты → после #144 mouth-чтения стали недостижимы вовсе;
      lip_fullness добавлен в allowlist неизмеримых + регресс-тест
      (2026-07-05)
- [ ] Бэклог (Qodo #145, optional): коллизия имени lip_fullness —
      в FaceMetrics это float-раскрытие рта, в FeatureAnswers это
      enum-толщина губ; переименовать (lip_gap / lip_thickness) при
      следующем breaking-change API, вместе с анатомической метрикой
- [ ] Анатомическая толщина губ: внешние точки (0→13 верх, 14→17 низ)
      / face_h, с калибровкой порогов; вернуть mouth-чтения от неё
- [ ] Межкадровая вариативность (мультифото): размах lip-gap и др. как
      самостоятельная черта «подвижность мимики» vs «константа»
