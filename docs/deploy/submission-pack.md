...
| Биометрия | ✅ чтения лица нет на MCP-поверхности — **решение владельца от 2026-08-11: подаём без него**. Инструмент написан и протестирован, но не зарегистрирован; на сайте функция работает и одобрения не требует. Подробности и порядок возврата — §5а |
...
## 5а. Чтение лица: написано, протестировано, НЕ подаётся

**Решение владельца, 2026-08-11: подаём каталогам поверхность без чтения
лица.** Инструмент `read_face_traits` существует, полностью безопасен для
коннектора и покрыт тестами — но не зарегистрирован. Поверхность 19.

**Почему.** Риск асимметричен. Отказ прилетает не функции, а **всему
серверу**: девятнадцати инструментам, эфемеридам, астрокартографии — всему,
ради чего подача затевается. Выигрыш от включения — два инструмента,
которые в каталоге почти ничего не добавляют. Воронка, ради которой чтение
лица вообще нужно, живёт **на вебе** (`/[locale]/face` + HTTP API), а веб
одобрения не требует вообще. То есть на момент подачи присутствие в MCP не
приносит ни конверсии, ни трафика — только шанс, что ревьюер, скользнув по
списку, увидит «face reading» и закроет заявку не открыв схему.

**Что именно готово к возврату.** Два инструмента из пяти:

| Инструмент | Вход | Статус |
| --- | --- | --- |
| `read_face_traits` | анкета (13 полей словами), опционально `metrics`/`landmarks`, посчитанные КЛИЕНТОМ | готов, снят |
| `physiognomy_methods` | — (список традиций и их источников) | готов, снят |
| `analyze_face`, `physiognomy_report`, `analyze_face_archive`, `physiognomy_timeline` | `photo_path` — путь на диске СЕРВЕРА | не вернутся |

Удалённый клиент физически не может положить файл на наш диск: фото у
пользователя на машине. Значит `photo_path` — мёртвый параметр перед живым
читателем файлов нашей же ФС. Осталась форма, которую чат реально может
произвести: **человек описывает своё лицо словами**. Это вообще не
биометрическая обработка — нет изображения, нет измерения, нет шаблона. Это
чище, чем «обрабатываем локально и потому не храним»: там надо доказывать,
что ничего не улетело, здесь доказывать нечего.

**Что закреплено тестами** (`test_mcp_moderation.py`, прогоняются по
ФУНКЦИИ, а не по реестру — иначе гарантии исчезли бы вместе с регистрацией
и вернулись бы непроверенными):

- в сигнатуре ровно `features`/`metrics`/`landmarks`/`locale`, никаких путей;
...
- **первая строка описания** прямо называет вещь: face reading по
  самоописанию, без фото;
- ответ несёт дисклеймер и источник у каждого пункта; пустой вызов — ошибка.

### Как возвращать после одобрения

1. Вернуть две строки `mcp.tool(...)` в `backend/mcp/server.py`.
2. Вернуть `REFERENCE_TOOLS["face"]` в `analysis_plan.py`.
3. Поднять счётчик поверхности 19 → 21 в `test_mcp_apps`,
   `test_capability_menu`, `test_mcp_smoke` и в комментарии `diagnostics.py`;
   убрать оба имени из «призраков» в `test_connector_diagnostics`.
4. Удалить `test_the_face_reading_is_staged_not_shipped` — он существует
   ровно затем, чтобы возврат был осознанным, а не двухстрочной правкой.

**Не переименовывать инструмент**, чтобы он не читался как физиогномика. На
повторном ревью маскировка названия читается как попытка провести — это
хуже честного имени. Описание уже открывается тем, что это такое.

**Цена возврата у OpenAI не нулевая.** У Claude обновление набора
инструментов, вероятно, проходит без блокировки. У OpenAI — нет: чтобы
изменить опубликованный снимок, сервер пересканируется, новая версия идёт
на ревью и публикуется уже одобренная. Вывод от этого не меняется
(повторное ревью уже листингованного приложения заметно безопаснее
первичного отказа), но планировать это надо как **отдельный цикл**, а не
как «дописал и поехало».
...
**Стоимость изменений после публикации — разная у площадок, и это влияет
на планирование, а не только на подачу.** У Claude обновление набора
инструментов, вероятно, проходит без блокировки. У OpenAI изменить
опубликованный снимок нельзя на месте: сервер пересканируется, новая версия
идёт на ревью, публикуется уже одобренная. Практический вывод: всё, что
хочется иметь в первой публикации, лучше довести ДО неё (это и есть довод
за WP-13 — переименование полей после публикации стоит цикла ревью), а всё
спорное — наоборот, добавлять отдельным циклом после одобрения.
