...
- §4.2 **Фреймворки толкования:**
  - Hall/Van de Castle (1966) — контент-анализ. **Кодирование структурное**
    (`hvdc_coder.py`): персонажи = различимые существительные (не местоимения),
    социальный акт требует цели, отрицание снимает/перекодирует акт
    («не смог» → failure), good fortune ≠ success (усилие перекодирует).
    Каждый счётчик несёт evidence-клаузу и цитату правила — детерминизм 1.0.
  - Юнгианские архетипы — символика
  - REM/NREM — нейрокогнитивный контекст
  - DreamBank — норм-сравнение по полу. **0/0 — неопределённость**: индикатор
    без данных уходит в `insufficient_data` с причиной, никогда не в 0.00;
    единицы отклонения явные (`deviation_unit`).
  - Личная серия (Domhoff) — сравнение с собственной базой пользователя,
    честный порог N≥15 (`series.py`).
