...
### 2026-08-12 (3) — холодный старт измерен и убран; дыра в WP-13; ПОПРАВКА к вчерашнему выводу

**Поправка, и она важная.** Вчера я записал «прод отставал на 12 дней».
Владелец нашёл улику, которая это опровергает: у того ответа
`computed_at: 2026-07-31T06:23:06`, а `computed_at` — это настоящий wall
clock (`datetime.now(timezone.utc)` в `_meta.py`). Вместе с коммитом
`43c0b8d` от 30 июля это **согласованная пара**: ответ реально произведён
31 июля. Объяснение без аномалий — не устаревший деплой, а устаревший
транскрипт. Сигнализация из (2026-08-12) всё равно полезна — дыра настоящая,
— но диагноз был неверен, и я утверждал его уверенно.

**Холодный старт: измерен, а не предположен.** Прод: первый вызов после
рестарта **упал**, второй 11403 мс, третий через 6 секунд — 32.2 мс, вход
идентичен, `cache_hit: false` на обоих. Локально на SSD тот же разрыв:
первый расчёт карты 1977 — **2.370 мс**, третий — **0.006 мс**, отношение
**380×**. На сетевом диске Render это и есть одиннадцать секунд.

Причина точнее гипотезы владельца: `_startup_probe` считает **три тела на
J2000**. Карта 1977 года — это **двенадцать тел** в другой области тех же
файлов. Проба доказывает, что файлы читаемы (её работа), и не греет почти
ничего из того, что читает карта.

Починка: отдельная `warm_ephemeris()` — 12 тел × 3 даты (~1950/1980/2010),
вызывается из lifespan FastAPI и при старте MCP-сервера. Проба осталась
дешёвой: она бежит на КАЖДОМ импорте, включая тесты и CLI. Замер после:
прогрев 2.0 мс, первая карта 1977 — **0.137 мс** вместо 2.370.
`keepalive.yml` тут был бессилен по построению: пинг `/health` держит процесс
живым, но эфемериду не трогает.

**Дыра в WP-13, найденная владельцем.** `calculate_natal_chart` не выпускал
`rule_source_tier` вообще — провенанс жил только английской прозой в
`how_to_read`. И тест «все выпущенные тиры настоящие» проходил на нём
**вакуумно**: нечего проверять — значит всё валидно. Тест доказывал
корректность там, где поле есть, и молчал там, где его нет.
Теперь: у натала `rule_source_tier: "computed"` (весь пейлоад — один тир),
плюс явный список `_NO_TIER_BY_DESIGN` с причиной на каждый инструмент и два
теста — что список не содержит несуществующих имён и что **всё остальное
обязано** нести тир.

**Третий за сессию случай сломанной видимости.** `test_chart_core_contract.py`
не запускался ни одним воркфлоу — и был красным. Моя же починка `tz_source`
(#189) его и уронила, честно: фикстура давала двум дверям **разный вход** —
билдеру явную зону, MCP-пути никакой. Пока `tz_source` врал «explicit» обеим,
сравнение выглядело сходящимся. Починена фикстура (обе двери выводят зону), и
добавлено утверждение, что обе реально пошли выведенным путём — иначе
сравнение снова станет вакуумным. Файл и ещё два (`test_natal_houses_aspects`,
`test_geo_candidates_and_coords`) добавлены в CI.

**Безопасность `STAFF_ACCOUNTS`** (замечание владельца). Проверено: пути,
меняющего `user.email`, в коде нет — email пишется один раз при регистрации.
Эскалация сейчас невозможна. Но гарантия держится на ОТСУТСТВИИ фичи: в день,
когда кто-то добавит обычную форму смены email, любой аккаунт получит путь к
правам staff, и ничего нигде не упадёт. Поэтому AST-тест: никакое присваивание
`.email` / `.oauth_subject` в backend-исходниках, плюс проверка, что `is_staff`
читает ровно эти два поля, плюс закрытый дефолт на пустом аллоулисте.

**Дата конца переходного режима назначена** (замечание владельца, верное):
аддитивность без даты конца — это не переход, а два поля навсегда. В CLAUDE.md
записано: на `schema_version: 3` уходят `confidence` и `cache_hit` (последний
захардкожен `false` с WP-6 — кэша нет, поле не обозначает ничего).

### 2026-08-12 (2) — WP-13 сделан аддитивно: у каждого утверждения появилось ИМЯ тира

Долг с 2026-07-28, отложенный письменно. Причина сделать его сейчас —
ревью, а не совместимость: публикация замораживает имена полей, а у OpenAI
их изменение стоит цикла пересканирования и ревью.

**Суть.** `"confidence": 0.7` читают все — модель, пользователь, ревьюер —
как «70% что правда». Это никогда не значило вероятность; это значило «взято
с тира синтеза». На астрологическом сервере разница не косметическая: число,
похожее на правдоподобие, превращает чтение традиции в предсказание с
шансами — ровно то утверждение, которого продукт избегает.

Теперь рядом едет имя: `computed` · `cited_rule` · `statistical` ·
`symbol_dictionary` · `model_synthesis` и `unvalidated_tradition` (0.6) для
физиогномики — намеренно НИЖЕ самой низкой ступени лестницы.

**Аддитивно в буквальном смысле:** `confidence` остался на месте и не
изменился, ни один читающий его клиент не ломается. Тест утверждает обе
половины слова «аддитивно».

Решения, которые пришлось принять по дороге:
- **Тир — свойство СЛОЯ, а не числа.** Где вызывающий знает слой,
  используется `LAYER_TIER`; где есть только вкопанное число —
  `tier_for_confidence`, и она **бросает** на значении вне лестницы, а не
  подбирает ближайшее. Комбинированные значения `numeric_confidence` (0.95 от
  схождения жёстких слоёв) в таблицу не входят намеренно: сумма источников —
  не тир, и втискивание её в имя выдумало бы происхождение, которого у
  утверждения нет.
- **Тир утверждения — его сильнейший источник**, не среднее: складывание
  слабых источников не делает сильного.
- **archetypes — 0.9, а не 0.8**: эти записи называют источник, а цитируемое
  правило старше словарной статьи. Тир обязан переносить это различие, иначе
  переименование теряет информацию, которая в числе была.

**Тест написан по живым ответам, а не грепом исходников:** литералы пишутся
четырьмя разными способами (значение словаря, словарь словарей, дефолт
Pydantic, модульная константа), и регекс, покрывающий все четыре, сам
относится к тому классу вещей, что молча перестают совпадать. Первая версия
именно это и сделала — нашла 4 из 16 и упала на пороге.

### 2026-08-12 — три «бага прода» оказались одним: прод отставал на 12 дней

Владелец прислал подробный разбор с тремя находками: `calculate_natal_chart`
падает с `UndefinedTable: relation "users" does not exist`;
`astrocartography_scan` отдаёт Атланте 0.0 при живом контакте Уран→Asc 6°,
неотличимо от бесконтактного Чикаго; в провенансе нет версии Swiss Ephemeris
и DE431 вопреки критерию приёмки WP-1.

**Все три верны. Все три починены 2026-08-03 одним коммитом** `5c0bea8`,
который так и называется: `fix(backend): natal 500 (users table), scan
hiding contacts, provenance version`. Комментарий в `_result_to_dict`
называет тот же пример — Атланта против Чикаго.

Отчёт был сделан по сборке `43c0b8d` от **30 июля**, на 4 дня раньше
починки. Проверено вживую на текущем проде (`276dce94b653`): карта считается
целиком, `entitlement: gated/pro`, провенанс несёт
`Swiss Ephemeris 2.10.03 / JPL DE431 .se1 files (SWIEPH)`, у каждой строки
скана есть `total_significance` и `unweighted`.

**Настоящий дефект здесь не в коде.** Владелец потратил вечер на разбор
багов, исправленных девять дней назад, и никак не мог этого узнать: `/health`
всё это время честно отвечал `healthy` — он и был healthy. Устаревший деплой
не виден вообще ничем.

Починка: `/health` теперь несёт `commit`, а `keepalive.yml` (он и так пингует
каждые 10 минут) сравнивает задеплоенный коммит с веткой. Порог — **возраст,
а не расхождение**: деплой в полёте расходится на минуты, это норма; сутки —
это деплой, которого не было. Сутки и больше → `::error::` и упавший ран,
потому что предупреждение на зелёном шедуле не читает никто. Не-предок
(откат, ветка, force-push) → громкий warning без падения.

Тест исполняет то самое `sed`-выражение из воркфлоу на реалистичном payload:
проверка, живущая только в YAML, не проверяется никем, пока не понадобится.

**Открыто (решение владельца, не техническое):** гейт стоит только на
`calculate_natal_chart`. `vocation_map`, `money_contour` и
`astrocartography_*` считают ту же эфемериду по тому же человеку бесплатно.
Единый seam (`check_entitlement`) на месте — размазана не проверка, а
ПОЛИТИКА: какие инструменты считаются платными. Это вопрос цены, а не
архитектуры, и менять его молча нельзя.

### 2026-08-11 (4) — своя аналитика: счётчики без идентификаторов, privacy уточнена

Решение владельца: свой счётчик, без персональных данных и без сторонних
скриптов. Сторонний трекер — это передача данных третьей стороне, отдельный
раздел в privacy (для GA ещё и вывоз за пределы ЕС) и внешний домен на
фронте, который специально никуда не ходит. Ради конверсии одной воронки
несоразмерно.

**Что построено.** Таблица `funnel_counters`: `(event, day) → total,
returning_count`. Всё. Пять событий закрытым списком; неизвестное имя
отвергается, а не создаёт строку (открытый эндпоинт, создающий строку на
произвольную строку, — способ залить чужую базу из браузера). `EventIn`
запрещает лишние поля: клиент, приложивший идентификатор, получает 422, а не
тихое игнорирование.

**Приватность держится на отсутствиях, поэтому тесты проверяют отсутствия.**
Список колонок сверяется целиком, плюс поиск подстрок `user/session/device/
ip/addr/client/fingerprint/visitor/uid/token` в именах. `day` обязан быть
`Date`, а не `DateTime`: точное время события при низком трафике само по себе
близко к идентификатору. Возврат считается без идентификации — браузер
хранит у себя дату последнего визита, сравнивает сам и шлёт «да/нет».

**Найдено при этом:** `returning` — зарезервированное слово SQL, `CREATE
TABLE` падает и на SQLite, и на Postgres. Это сломало бы миграцию на проде;
поймано только потому, что тест реально создаёт таблицу. Колонка
переименована в `returning_count`.

**Заодно расшит миграционный тест:** он был прибит к имени головной ревизии
(`== "0002_chart_gate_identity"`), то есть каждая новая миграция требовала
правки этой строки — а строка, которую правят механически, перестаёт что-либо
утверждать. Теперь проверяется то, что действительно ломает деплой: голова
ровно одна (история не разветвилась) и она же самая новая.

**Privacy переписана, а не дополнена.** Пункт 5 говорил: «В сервисе нет
аналитики: ни Google Analytics, ни счётчиков, ни трекеров поведения». После
этой работы фраза стала бы ложной — в том самом документе, который ревьюер
читает первым. Теперь он называется «Счётчики и аналитика» и описывает
ровно то, что происходит; в пункт 6 добавлена строка про дату последнего
визита в localStorage. Дата обновления — 2026-08-11.

**Открыто:** формулировку про localStorage стоит показать юристу вместе с
остальной правкой — правило про хранение на устройстве в ЕС шире GDPR и в
некоторых трактовках цепляет даже localStorage. Одна строка в том же заходе.

### 2026-08-11 (3) — экран лица пересобран в три слоя: измерение → граница → традиция → переход

Раскладка владельца, реализована. Суть не в косметике: **граница между
измеримым и традицией сделана видимой раньше, чем её прочитают** — сменой
гарнитуры (моно → UI-шрифт) и одной строкой нормального кегля на самом
переходе, а не 8px серым в подвале.

Следствие раскладки, которое пришлось признать: **заголовок «Тип лица:
Металл + Вода» — это мяньсян, а не измерение**, и он стоял первым, оформленный
как ГЛАВНЫЙ результат. Пришлось увести его вниз, под границу. Раскол честен
только если самое заметное утверждение ему подчиняется.

Порядок теперь: `измерено` (кадры) → подпись лица (σ-отклонения, моно) →
что измеримо, а что нет → **граница** → тип лица → сводные оценки → чтения
прозой → дисклеймер → пересканировать → **переход**.

**Слой перехода** (`FaceTransition.tsx`) — сравнение, а не «купить»:
«Лицо не меняется. Небо меняется каждый день.» Форма ввода даты стоит
ПРЯМО НА ЭКРАНЕ результата, а не за кликом: каждое лишнее перелистывание
между намерением и вводом стоит примерно половины людей. Использована
готовая `BirthDataForm` («Моё рождение — в ___ в ___ в ___»), которая сама
пишет в localStorage по мере набора, поэтому `/natal` подхватывает ответ без
query-параметров и считает сразу. Email не спрашивается нигде — тест на это.

**`/face` убран из главной навигации.** Он вход, а не раздел: в меню рядом с
натальным колесом он заявляет себя равным инструментом и размывает ровно то,
что весь остальной каркас пытается утверждать. На главной его тоже нет —
приходят по внешней ссылке.

Не сделано и почему:
- **картинка для расшаривания** (композит: контур лица + сетка в брассе,
  две-три цифры моно, подпись) — отдельная работа на canvas, не начата;
- **четыре метрики воронки** — в проекте НЕТ никакой аналитики. Это не
  «дописать вызов»: политика приватности сейчас говорит «по умолчанию не
  хранится ничего», и любой трекинг делает эту фразу ложной. Нужно решение
  владельца (свой счётчик событий без персональных данных? сторонний?) и
  правка `/legal/privacy` — до этого ничего не добавляю.

### 2026-08-11 (2) — решение владельца: подаём БЕЗ чтения лица. Поверхность обратно 19

Владелец взвесил и решил: в каталоги идём без физиогномики. Аргумент —
асимметрия риска. Отказ прилетает не функции, а **всему серверу**:
девятнадцати инструментам, эфемеридам, астрокартографии. Выигрыш от
включения — два инструмента, которые в каталоге почти ничего не добавляют.
Воронка, ради которой чтение лица нужно, живёт на вебе, а веб одобрения не
требует вообще. То есть на момент подачи присутствие в MCP не приносит ни
конверсии, ни трафика — только шанс, что ревьюер закроет заявку, увидев
«face reading» и не открыв схему.

Поэтому две строки регистрации сняты, `REFERENCE_TOOLS["face"]` убран,
счётчик везде обратно 19, из `server.json` убрана фраза про чтение лица
(описание не должно обещать то, чего на поверхности нет).

**Ключевое в том, КАК снято.** Инструмент не откачен — он написан, работает
и покрыт тестами; тесты только переехали с реестра на саму функцию. Если бы
они шли через `mcp.list_tools()`, то вместе с регистрацией исчезли бы и
гарантии, и через несколько месяцев кто-то вернул бы две строки, добавив на
публичную поверхность то, что никто давно не проверял. Теперь проверяются
сигнатура (никаких путей к файлам), три обязательных утверждения в описании
и поведение — независимо от того, зарегистрирован инструмент или нет.

Само решение тоже сделано исполняемым:
`test_the_face_reading_is_staged_not_shipped` падает, если инструменты
вернулись. Его удаление — предусмотренный способ пройти дальше, но удалять
придётся осознанно, прочитав почему он был. Порядок возврата расписан в
`submission-pack.md §5а`.

Плюс два уточнения от владельца, оба записаны:
- **Не переименовывать** инструмент, чтобы он не читался как физиогномика.
  На повторном ревью маскировка названия читается как попытка провести —
  хуже честного имени. Первая строка описания теперь обязана прямо называть
  вещь, это отдельный тест.
- **У OpenAI обновление опубликованного набора инструментов не бесплатно**
  (я утверждал обратное): сервер пересканируется, новая версия идёт на
  ревью. Вывод не меняется, но возврат планируется как отдельный цикл.
  Побочное следствие — довод за WP-13 ДО публикации: переименование полей
  после неё стоит ровно такого же цикла.

### 2026-08-11 — чтение лица вернулось на MCP-поверхность (19 → 21), в другой форме

Владелец: «19 инструментов, физиогномики среди них нет — Я БЫ ХОТЕЛ ЕЕ
ВЕРНУТЬ». Вернул. Не тем же составом, что уходил в WP-10, и причина
техническая, а не вкусовая.

- **Что вернулось:** `read_face_traits` (анкета из 13 полей словами;
  опционально `metrics`/`landmarks`, посчитанные клиентом) и
  `physiognomy_methods` (список традиций с источниками). Оба `readOnlyHint`,
  `openWorldHint=false`.
- **Что осталось снятым и почему:** `analyze_face`, `physiognomy_report`,
  `analyze_face_archive`, `physiognomy_timeline` — все берут `photo_path`,
  путь на диске СЕРВЕРА. Удалённый клиент туда файл положить не может: фото
  у пользователя на машине. То есть параметр мёртв для честного применения и
  жив только как способ направить наш файловый читатель на нашу же ФС.
  Он защищён (`_safe_read_path`, ужесточён в #136–#139), но защищённая дверь
  в пустую комнату лучше закрыта. `physiognomy_report` вдобавок пишет HTML и
  возвращает путь, который некому открыть, — ровно за это WP-8 его и снял.
- **Почему анкета вообще проходит по политикам.** Нет изображения, нет
  измерения, нет шаблона — это не биометрическая обработка. Человек
  описывает своё лицо своими словами и получает традиционное чтение. Аргумент
  держится ровно до тех пор, пока в реестре нет фото-инструментов, поэтому
  за этим следит тест (`test_the_face_tools_never_take_a_server_side_path`),
  а не соглашение.
- **Позиция по кадрам не изменилась.** Оценка ДРУГОГО человека — найм,
  кредит, страховка, аренда, полиция — записана в описание самого
  инструмента (модель читает его до вызова) и закреплена тестом. EU AI Act
  ст. 5 + корреляция черт лица с защищёнными признаками; продуктовый
  дисклеймер это и так запрещал.
- **Риск подачи назван, не спрятан:** `docs/deploy/submission-pack.md §5а`.
  Ревьюер вправе прочитать «face reading» как запрещённую категорию, не
  открывая схему, и отказ получит ВЕСЬ сервер. Выбор — подать без чтения лица
  и добавить после одобрения, либо подать как есть. Это решение владельца; по
  умолчанию действует второе (то, что в коде).
- Сведено: реестр 21 в четырёх местах, где число было прибито
  (`test_mcp_apps`, `test_capability_menu`, `test_mcp_smoke`, комментарий в
  `diagnostics.py`); `physiognomy_methods` убран из списка «призраков» в
  `test_connector_diagnostics` — теперь он служится по-настоящему и ничего
  про кэш клиента не доказывает, его место заняли `analyze_face` и
  `physiognomy_report`. `REFERENCE_TOOLS` получил домен `face`: чтение лица —
  словарный поиск, не шаг разбора, поэтому меню оно не носит.

**Попутно: main был красный, и это моя ошибка из PR #188.** Когда
`_dcr_advertised` получил аргумент `probe`, три теста в
`test_connector_diagnostics.py` продолжали подменять его заглушкой
`async def _dcr()` без аргументов. `monkeypatch.setattr` подменяет атрибут,
не проверяя, что вызывается он так же, — поэтому расхождение вылезло не в
месте подмены, а как 500 из эндпоинта, с настоящим `TypeError` под пятью
кадрами starlette. Локально не видно вообще: файл целиком скипается, когда
в песочнице нет необязательной зависимости бэкенда, — а это её нормальное
состояние. Зелено локально, красно в CI, и красно с момента merge.

Починка — не «дописать `probe=False` в три заглушки»: сигнатура заглушки
теперь живёт в одном месте (`_dcr_stub`), а
`test_the_dcr_stub_matches_the_real_probe` сравнивает её с настоящей через
`inspect.signature`. Важное свойство: этот тест не просит фикстуру `client`,
поэтому переживает тот самый скип, который прятал поломку.

Урок шире одного файла: **скип — не зелёный.** Если файл может целиком
выпасть из прогона из-за отсутствующей зависимости, его инварианты стоит
проверять хотя бы одним тестом, которому приложение не нужно.
