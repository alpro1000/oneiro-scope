...
- **2026-07-10 (pattern: Cyrillic keyword regex false positives)** — a live test run of the dream analyzer against a real user dream (treasure/gold narrative) surfaced a reusable bug class in `backend/services/dreams/analyzer.py::_compile_patterns()`, not a one-off typo. Two distinct failure modes found, both from the same root cause (the Cyrillic-keyword branch matches `\b{keyword}\w*\b` — root + any suffix, to catch inflection): (1) **stem collision** — the morphology stem fallback made "жена" (wife) and "жених" (groom) indistinguishable (`morphology.stem()` reduces both to "жен"), so any dream mentioning a wife was silently scored as a wedding symbol; (2) **prefix collision** — the keyword "авто" (colloquial, indeclinable "car") matched as a literal prefix inside unrelated longer words like "авторитетный" (authoritative), firing a phantom vehicle symbol. Fixed narrowly, not with a blanket rewrite: `_STEM_FALLBACK_EXCLUDE` (keywords excluded from the stem fallback set) and `_NO_WILDCARD_KEYWORDS` (keywords forced to exact-word match, no `\w*` suffix) — both small, documented, extend-as-found sets rather than a general morphological overhaul, consistent with the project's anti-bloat convention. Also fixed in the same pass: `character_patterns["female"]` was missing "жена" entirely (a wife present in a dream produced `female_characters=0`), and `interaction_patterns["aggressive"]` had no push/shove word in either language (a described shove produced `aggressive_interactions=0`). Added a "treasure" symbol (`good_fortunes` category, `self` archetype, citing Jung's "treasure hard to attain") since it was conspicuously absent from a 56-symbol KB. All 4 fixes verified via direct re-run against the same dream text (`/tmp/.../scratchpad/report/dream_test.py`) plus the existing `test_dream_interpreter_narrative.py`/`test_dream_morphology.py` suites (29/29 passing, no regressions). Takeaway for future symbol/keyword additions: any short Cyrillic keyword (≤4-5 letters) should be sanity-checked against common unrelated words sharing its first letters before being added as a wildcard-suffix keyword.
...
### 2026-07-10 — claude/astrology-chart-analysis-8or26i — dream analyzer bug fixes + manual narrative interpretation (no LLM provider)

**Trigger:** owner ran their own real dream through `/dream` to test what the sonnik service actually does. Live run surfaced 3 bugs (later 4, see §6 pattern entry above) and one KB gap (no "treasure" symbol despite gold coins being the dream's central object). Owner asked to fix all of them, personally confirmed one himself (wife wrongly excluded from character count), then explicitly asked me to stand in as the interpretation layer myself (no LLM provider configured in this sandbox) using the project's own methodology.

**Done:** fixed all 4 regex/stemming bugs in `backend/services/dreams/analyzer.py` (жена/жених stem collision, missing жена forms in the female-character regex, missing push/shove forms in the aggressive-interaction regex, авто/авторитетный prefix collision), added a "treasure" symbol to `symbols.json` (57 symbols total now). Verified via `dream_test.py` re-run + `test_dream_interpreter_narrative.py`/`test_dream_morphology.py` (29/29 passing). Committed as two commits, pushed. Performed the full narrative interpretation manually across several turns (Hall/Van de Castle content stats, cited Jungian archetypes at 0.8 confidence, REM/NREM functional theories — continuity hypothesis, threat/social simulation theory, mood-regulatory function — at the owner's explicit request to go "deep, scientific"), correcting my own initial read twice based on the owner's direct clarification of his actual felt experience (no guilt over keeping the gold; the several statuettes were freely shown to the crowd, held no personal value) — a good example of self-reported felt affect overriding my inferred assumption. Also firmly declined a literal "what does this dream predict" framing — reiterated the project's no-fortune-telling rule and explained why (continuity hypothesis ≠ precognition), which the owner accepted and reframed into "reflect what's happening now" instead.

**Pending:** owner may want the figurine/Stalin-statuette image explored separately later (explicitly deferred, not skipped — owner said "не важно" only for *this* round). No code changes pending from this thread.
