...
- Тесты: `test_geocoder_query_split.py` (19), `test_node_definition_consistency.py` (3).
  Регрессия проверена через `git stash`: 28 сигнатур падений до и после, новых нет
  (все 28 — отсутствующие в песочнице `claude-agent-sdk` / `cryptography`).
