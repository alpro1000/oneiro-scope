...
**Последняя ветка работы:** `claude/photo-personality-analysis-2jy29b` (design-system Phase 1+2 + astrocartography page; НЕ замержена — PR по запросу owner)
**Текущий main HEAD:** physiognomy (auto-zoom, signature, trait verdicts, life-context, скилл /face-portrait) — всё замержено

**Новое в сессии 2026-07-07 (вечер): интерактивная карта астрокартографии встроена в сайт**
- Owner вспомнил, что карта астрокартографии уже делалась — нашёл её:
  `frontend/public/astrocartography.html`, orphan-файл из PR #130, НИКОГДА
  не подключённый к реальному сайту (без роута, без пункта меню, без
  токенов) и с ХАРДКОДОМ реальных данных рождения владельца (дата/время/
  Запорожье) как дефолт публичной статической страницы. Файл удалён
  (`git rm`) — PII не должен раздаваться статикой.
- Построена настоящая страница `/[locale]/astrocartography`:
  - Новый клиент `lib/astrocartography-client.ts` → реальные бэкенд-эндпоинты
    `POST /astrocartography/chart` (GeoJSON линий + геометрия) и
    `POST /astrocartography/point` (4 угла + контакты + вердикт при клике) —
    вместо клиентской JS-астрономии старого файла.
  - Новая зависимость `leaflet` + `react-leaflet` (не было в проекте);
    `components/AstrocartographyMap.tsx` — динамически импортируется
    (`next/dynamic`, `ssr:false`), т.к. Leaflet требует window.
  - Форма рождения: дата/время/таймзона (свой select, НЕ переиспользован
    `TimezoneSelector` — он пишет в тот же localStorage-ключ, что и лунный
    виджет, что перезаписало бы пользовательский часовой пояс календаря)
    + `CityAutocomplete` (даёт lat/lon, которых натальная форма не даёт —
    astrocartography сам не геокодирует).
  - Клик по карте → `FindingCard` (углы-измерение → вердикт правила →
    что усиливает/на что обратить внимание) + `ConfidenceBadge` (0.8,
    "relocation rule-set") + 4 карточки углов (Asc/MC/IC/Desc) с контактами.
  - Пункт "Карта мест" в `Header.tsx` (после Астрологии); переводы
    `AstrocartographyPage` (ru/en полностью) + `Header.map` во всех 5
    локалях (de/es/fr — только пункт меню, как раньше у FacePage: полный
    контент страницы для тех 3 локалей — бэклог, не регрессия).
  - **Реальный баг найден и исправлен во время сквозной проверки:** цвет
    линии по планете брался БЕЗ `.toLowerCase()` (бэкенд отдаёт "Sun",
    "Saturn" с большой буквы, карта словаря — строчными) → все 10 линий
    рисовались одним фиолетовым цветом вместо палитры по планетам.
    Пойман только благодаря живому прогону с реальным бэкендом
    (не всплыл бы в tsc/jest/mock-скриншотах).
  - **Проверено сквозным прогоном с настоящим FastAPI-бэкендом** (не только
    моками): поднят локально (`sqlite+aiosqlite`), реальные вызовы
    `/cities/search`, `/astrocartography/chart`, `/astrocartography/point`
    — форма → карта строится → клик по карте → полная панель вердикта
    рендерится с реальными данными (Плутон/Луна/Венера на углах,
    "чистая удача" бейдж). Тайлы CARTO не грузятся В ЭТОЙ песочнице
    (сетевой прокси блокирует внешний CDN) — это ограничение окружения,
    не баг; сами линии ACG не зависят от тайлов и рисуются корректно.
  - tsc чисто, jest 34/34 (без изменений — Leaflet-карта не тестируется
    юнитами, как и другие browser-API-компоненты в этом репо: FaceScanner,
    VoiceInput, CityAutocomplete), `next build` зелёный
    (astrocartography route 6.83 kB).
