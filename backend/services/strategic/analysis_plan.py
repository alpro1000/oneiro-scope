...
#
# A "face" domain belongs here the day face reading returns to the MCP surface
# (see the block in `backend/mcp/server.py`): a face reading is a dictionary
# lookup keyed by what the reader said about their own face — it computes
# nothing and leads nowhere, so it is a reference, not a stage. It is absent
# now only because the tools are absent; every name in this table must be
# registered, which `test_reference_lookups_are_registered_too` enforces.
